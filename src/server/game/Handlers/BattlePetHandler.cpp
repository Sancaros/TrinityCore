...
void WorldSession::HandleBattlePetModifyName(WorldPackets::BattlePet::ModifyName& packet)
{
    ...
    if (creature->GetBattlePetCompanionGUID() == packet.BattlePetGUID)
    {
...
void WorldSession::HandleBattlePetSetSlot(WorldPackets::BattlePet::SetBattleSlot& packet)
{
    ...
    if (packet.SlotIndex >= MAX_PET_BATTLE_SLOTS)
        return;
...
void WorldSession::SendPetBattleSlotUpdates(bool newSlotUnlocked /*= false*/)
{
    ...
    if (unlockedSlotCount)
        _player->SetPlayerFlags(PLAYER_FLAGS_PET_BATTLES_UNLOCKED);
...
void WorldSession::SendPetBattleInitialUpdate(PetBattle* petBattle)
{
    ...
    {
        if (auto trainer = ObjectAccessor::FindUnit(petBattle->InitialWildPetGUID))
        {
    ...
    for (uint8 i = 0; i < MAX_PET_BATTLE_TEAM; i++)
    {
        ...
        for (uint8 v = 0; v < petBattle->Teams[i]->TeamPetCount; v++)
        {
            ...
            petUpdate.JournalInfo.CollarID = pet->DisplayModelID;
...
void WorldSession::SendPetBattleQueueStatus(uint32 ticketTime, uint32 tcketID, uint32 status, uint32 avgWaitTime)
{
   ...
   // statusUpdate.Msg.Ticket.Type = WorldPackets::LFG::RideType::PvPPetBattle; TODO LUIS
...
void WorldSession::SendBattlePetUpdates(BattlePet* pet2 /*= nullptr*/, bool add)
{
    ...
    if (pet2)
    {
        petUpdate.Guid = pet2->JournalID;
        petUpdate.Species = pet2->Species;
        if (auto const& speciesInfo = sBattlePetSpeciesStore.LookupEntry(pet2->Species))
            petUpdate.CreatureID = speciesInfo->CreatureID;
        petUpdate.CollarID = pet2->DisplayModelID;
        petUpdate.Breed = pet2->Breed;
        petUpdate.Level = pet2->Level;
        petUpdate.Exp = pet2->XP;
        petUpdate.Flags = pet2->Flags;
        ...
        petUpdate.Quality = pet2->Quality;
        petUpdate.Name = pet2->Name;
    ...
    else
    {
        for (auto const& itr : *pets)
        {
            ...
            if (auto const& speciesInfo = sBattlePetSpeciesStore.LookupEntry(pet->Species))
                petUpdate.CreatureID = speciesInfo->CreatureID;
            petUpdate.CollarID = pet->DisplayModelID;
...
void WorldSession::SendBattlePetJournal()
{
    ...
    for (auto const& pet : *pets)
    {
        ...
        WorldPackets::BattlePet::BattlePet info;
        info.Guid = v->JournalID;
        info.Species = v->Species;
        if (auto speciesInfo = sBattlePetSpeciesStore.LookupEntry(v->Species))
            info.CreatureID = speciesInfo->CreatureID;
        info.CollarID = v->DisplayModelID;
        info.Breed = v->Breed;
        info.Level = v->Level;
        info.Exp = v->XP;
        info.Flags = v->Flags;

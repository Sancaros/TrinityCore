...
{
    JoinBracket(BRACKET_TYPE_BRAWL_BATTLEGROUND, pvpBrawlInfo.RolesMask);
}
...
void WorldSession::JoinBracket(uint8 slot, uint8 rolesMask /*= ROLES_DEFAULT*/)
{
   /* Player* player = GetPlayer();
    ...
    bgTypeId = bg->GetTypeID();
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, BattlegroundQueueIdType::Arena, true, Jointype);
    ...
    if (!err)
    {
        TC_LOG_DEBUG("bg.battleground", "Battleground: leader %s join type %u", player->GetName(), Jointype);

        //matchmakerRating = grp->GetAverageMMR(BracketType(slot));
          //WorldPackets::Battleground::IgnorMapInfo ignore;

          ginfo = bgQueue.AddGroup(player, grp, bgTypeId, bracketEntry, Jointype, true, false, 0, matchmakerRating);
          avgTime = bgQueue.GetAverageQueueWaitTime(ginfo, bracketEntry->GetBracketId());
    ...
    else
    {
        ...
        TC_LOG_ERROR("network", "Battleground: Join Fail. QueueTypeID: %u, JoinType: %u, Slot: %u, Err %u", bgQueueTypeId, Jointype, slot, err);
        return;
    }
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        sBattlegroundMgr->BuildBattlegroundStatusQueued(&queued, bg, member, member->AddBattlegroundQueueId(bgQueueTypeId), ginfo->JoinTime, bgQueueTypeId, avgTime, 0, true);
...
void WorldSession::HandleBattlemasterJoinArenaSkirmish(WorldPackets::Battleground::BattlemasterJoinArenaSkirmish& packet)
{
   /* bool isPremade = false;
    ...
    uint32 arenaType = ARENA_TEAM_2v2;
    if (grp && grp->GetMembersCount() >= 5)
        arenaType = ARENA_TEAM_3v3;
    ...
    else
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            WorldPackets::Battleground::BattlefieldStatusQueued battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusQueued(&battlefieldStatus, bg, _player, queueSlot, ginfo->ArenaType, bgQueueTypeId, avgTime, 0, false);
            member->SendDirectMessage(battlefieldStatus.Write());
            TC_LOG_DEBUG("bg.battleground", "Battleground: player joined queue for bg queue type %u bg type %u, %s, NAME %s",
                bgQueueTypeId, bgTypeId, member->GetGUID().ToString().c_str(), member->GetName().c_str());
        }

        ...
        for (size_t currentPetID = 0; currentPetID < Teams[currentTeamID]->TeamPetCount; ++currentPetID)
        {
            ...
            if (winnerTeamID == currentTeamID && availablePetCount && BattleType == PET_BATTLE_TYPE_PVE && currentPet->IsAlive() && currentPet->Level < BATTLE_PET_MAX_LEVEL && FightedPets.find(currentPet->ID) != FightedPets.end())
            {
                ...
                if (currentPet->XP + xpEarn >= xpToNextLevel)
                {
                    ...
                    {
                        player->UpdateCriteria(CriteriaType::BattlePetReachLevel, currentPet->Level, speciesInfo->PetTypeEnum, currentPet->Species);
                        player->UpdateCriteria(CriteriaType::ActivelyEarnPetLevel, speciesInfo->ID, currentPet->Level, currentPet->Species);
                    }
            ...
            if (winnerTeamID == currentTeamID)
            {
                ...
                player->UpdateCriteria(CriteriaType::WinPetBattle, BattleType != PET_BATTLE_TYPE_PVE, 0, speciesInfo ? speciesInfo->ID : 0);
                /*if (speciesInfo)
            ...
            if (BattleType == PET_BATTLE_TYPE_PVE && currentTeamID == winnerTeamID && Teams[currentTeamID]->CapturedPet != PET_BATTLE_NULL_ID)
            {
                ...
                {
                    player->UpdateCriteria(CriteriaType::AccountKnownPet, speciesInfo->CreatureID);
                    player->UpdateCriteria(CriteriaType::LearnedNewPet, speciesInfo->ID, 0, speciesInfo->ID);
                    player->UpdateCriteria(CriteriaType::AccountObtainPetThroughBattle, 1, Pets[Teams[currentTeamID]->CapturedPet]->Quality, speciesInfo->ID);
                }
            ...
            /*if (BattleType == PET_BATTLE_TYPE_PVE && PveBattleType == PVE_PETBATTLE_TRAINER && currentTeamID == winnerTeamID)
